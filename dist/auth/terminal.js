...
export const GITHUB_LOGIN_REQUIRES_BROWSER_MESSAGE = "GitHub login requires a browser. Run `ccr login` (without --terminal).";
...
export function resolveEndpoint(env = process.env, explicitEndpoint) {
    return normalizeEndpoint(explicitEndpoint ?? env.CCR_ENDPOINT ?? DEFAULT_ENDPOINT);
}
export function authFilePath(homeDir = os.homedir()) {
    return path.join(homeDir, ".ccr", "auth.json");
}
export async function writeAuthFile(record, options = {}) {
...
function buildSignupOrLoginUrl(endpoint) {
...
function TerminalAuthApp({ endpoint, fetchImpl, homeDir, fsImpl, maxAttempts, onFinish, }) {
    const { exit } = useApp();
    ...
    const submitPassword = useCallback(async (submittedPassword) => {
        setError(null);
        setStep("submitting");
        try {
    ...
    const passwordDisplay = step === "password"
        ? React.createElement(TextInput, {
            value: password,
        ...
        : step === "submitting"
            ...
            : null;
    return React.createElement(Box, { flexDirection: "column" }, React.createElement(Text, { bold: true }, "CCR sign in / sign up"), React.createElement(Box, { marginTop: 1 }, React.createElement(Text, null, "Email: "), emailDisplay), step !== "email"
        ? React.createElement(Box, null, React.createElement(Text, null, "Password: "), passwordDisplay)
        : null, error
        ? React.createElement(Box, { marginTop: 1 }, React.createElement(Text, { color: "red" }, error))
        : null, step === "submitting"
        ? React.createElement(Box, { marginTop: 1 }, React.createElement(Text, { color: "cyan" }, React.createElement(Spinner, { type: "dots" }), " Signing in..."))
        : null, finalMessage
        ? React.createElement(Box, { marginTop: 1 }, React.createElement(Text, { color: step === "success" ? "green" : "red" }, finalMessage))
        : null);
}
export async function runTerminalAuth(options = {}) {
    ...
    const app = render(React.createElement(TerminalAuthApp, {
        endpoint,
        fetchImpl,
        homeDir,
        fsImpl,
