...
function truncate(s, limit = MAX_TOOL_OUTPUT) {
    if (s.length <= limit)
        return s;
    ...
    async run(ctx, { path: target, edits }) {
        ...
        for (let i = 0; i < edits.length; i++) {
            ...
            if (!cur.includes(old_string))
                return `ERROR: edit #${i + 1}: old_string not found`;
        ...
        if (!ok)
            return "DENIED: user rejected the edits.";
...
const insertLinesTool = {
    ...
    async run(ctx, { path: target, line, content }) {
        ...
        if (line < 1 || line > lines.length + 1)
            return `ERROR: line ${line} out of range (1..${lines.length + 1})`;
        ...
        if (!ok)
            return "DENIED: user rejected the insert.";
...
const grepTool = {
    ...
    async run(ctx, { pattern, path: base, glob }) {
        ...
        const out = [];
        for await (const file of walk(root)) {
            if (glob && !fnmatch(path.basename(file), glob))
                continue;
            let text;
            try {
    ...
    async run(ctx, { command, timeout = 120 }) {
        ...
        try {
            ...
            if (err.stderr)
                parts.push("--- stderr ---\n" + err.stderr);
            if (err.killed)
                parts.push(`(killed: ${err.signal ?? "timeout"})`);
...
const askUserQuestionTool = {
    ...
    async run(ctx, { questions }) {
        ...
        if (questions.length > 3)
            return "ERROR: at most 3 questions per call";
        for (let i = 0; i < questions.length; i++) {
            const q = questions[i];
            if (!q || typeof q.question !== "string")
                return `ERROR: question ${i} missing 'question' field`;
            if (!Array.isArray(q.options))
                return `ERROR: question ${i} 'options' must be an array`;
        }
