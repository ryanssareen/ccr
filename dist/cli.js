...
import { buildClient, DEFAULT_MODEL, initialMessages, runAgent, } from "./agent.js";
import { listSessions, loadSession, newSessionId, saveSession } from "./session.js";
import { App } from "./app.js";
import { applyConfig, loadAuth, loadConfig } from "./config.js";
...
async function loadProjectContext(root) {
    ...
    for (const name of CONTEXT_FILES) {
        ...
        const piece = `--- ${name} ---\n${body}${note}`;
        if (total + piece.length > PROJECT_CONTEXT_TOTAL)
            break;
        chunks.push(piece);
        total += piece.length;
    }
...
function consoleApprover(mode, rl) {
    return async (req) => {
        if (mode === "bypass") {
        ...
        const answer = await new Promise((res) => rl.question("Approve? [y/N] ", res));
    ...
    return {
        ...
        toolCallEnd(name, result, isError) {
            ...
            if ((name === "glob" || name === "grep") && result !== "(no matches)") {
                ...
                if (lines.length > 3)
                    console.log(kleur.dim(`    … +${lines.length - 3} more`));
                return;
            }
        ...
        setStatus(text) {
            if (text)
                process.stdout.write("\r" + kleur.yellow(text) + "    ");
            else
...
async function runOneShot(args, root, auth) {
    let client;
    const quotaRef = { current: null };
    try {
    ...
    const ctx = { root, approve: consoleApprover(args.mode, rl) };
    const run = { client, model: args.model, ctx, reporter: consoleReporter() };
...
async function runInteractive(args, root, auth) {
...
async function runLogin(args) {
    ...
    try {
        ...
        const browserModule = (await import(browserModulePath));
...
async function main() {
    ...
    if (!auth && process.env.GROQ_API_KEY) {
        process.stderr.write(kleur.yellow("warning: GROQ_API_KEY direct mode is deprecated. Run `ccr login` to switch to the free managed service.\n"));
    }
    if (args.listSessions) {
        ...
        for (const s of sessions)
            console.log(`${path.basename(s, ".json")}\t${s}`);
        return 0;
    }
    if (args.print) {
