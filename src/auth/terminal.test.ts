...
async function typeAndSubmit(stdin: NodeJS.ReadStream, text: string): Promise<void> {
  for (const character of text) {
    (stdin as any).write(character);
    await new Promise((resolve) => setTimeout(resolve, 0));
  }
  (stdin as any).write("\r");
}
...
async function readAuth(homeDir: string): Promise<any> {
...
describe("terminal auth", () => {
  ...
  it("prints helpful error when --method github is combined with --terminal", async () => {
    ...
    assert.match(
      terminal.output(),
      new RegExp(GITHUB_LOGIN_REQUIRES_BROWSER_MESSAGE.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
  });
});
