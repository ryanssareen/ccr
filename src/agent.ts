...
export const DEFAULT_MODEL: string = process.env.CCR_MODEL || "llama-3.3-70b-versatile";

const MAX_ITERATIONS = 25;

const SYSTEM_PROMPT = (root: string, projectContext: string) =>
  `You are ccr, a terminal-native coding assistant operating inside the user's project directory.
...
function parseRetryAfterSeconds(err: any): number {
  ...
  if (hdrs) {
    const get =
      typeof hdrs.get === "function" ? (k: string) => hdrs.get(k) : (k: string) => hdrs[k];
...
async function step(run: AgentRun, messages: any[]): Promise<boolean> {
  ...
  let content = "";
  const tcByIndex = new Map<number, any>();
