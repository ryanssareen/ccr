...
type AssistantEntry = { kind: "assistant"; text: string };
type ToolEntry = {
  kind: "tool";
  name: string;
  argsPreview: string;
...
type SystemEntry = { kind: "system"; text: string; tone?: "info" | "warn" | "error" };
type Entry = UserEntry | AssistantEntry | ToolEntry | SystemEntry;
...
function QuotaLine({ quota }: { quota: QuotaState }) {
  ...
  return (
    <Text color={color} dimColor={dim}>
      quota {quota.used.toLocaleString()} / {quota.limit.toLocaleString()} · resets{" "}
...
function colorizeDiffLine(line: string) {
  if (line.startsWith("+++") || line.startsWith("---")) return <Text bold>{line}</Text>;
...
function ModePicker({
  ...
  const items = (["ask", "accept-edits", "bypass"] as Mode[]).map((m) => ({
    label:
      (m === current ? "● " : "  ") +
      m +
      "  " +
  ...
  return (
    ...
    >
      ...
      <Box marginTop={1} flexDirection="column">
        <SelectInput items={items} onSelect={(item) => onSelect(item.value as Mode)} />
      </Box>
      <Box marginTop={1}>
    ...
    <Box flexDirection="column" borderStyle="round" borderColor="cyan" paddingX={1} marginY={1}>
      ...
      <Box marginTop={1} flexDirection="column">
        {items.length === 0 ? (
          ...
          <SelectInput items={items} limit={8} onSelect={(item) => onSelect(item.value)} />
        )}
      </Box>
      <Box marginTop={1}>
        <Text dimColor>↑↓ navigate · Enter pick · Esc cancel</Text>
...
export function App(props: AppProps) {
  ...
  const approve = useCallback(
    (req: ApprovalRequest): Promise<boolean> => {
      ...
      if (m === "accept-edits" && req.kind === "edit") {
        pushEntry({
          kind: "system",
          text: `[accept-edits] auto-approve: ${req.title}`,
          tone: "warn",
        });
        return Promise.resolve(true);
      }
  ...
  const ask = useCallback((req: AskRequest): Promise<AskAnswer[]> => {
  ...
  const reporter: Reporter = {
    token: (s: string) => setStreaming((cur) => cur + s),
    ...
    iterationCap: () =>
      pushEntry({ kind: "system", text: "⚠ hit max iterations", tone: "warn" }),
    setStatus: (text: string | null) => setStatus(text),
    setQuota: (state: QuotaState) => setQuotaState(state),
  };
  ...
  return (
    <Box flexDirection="column">
      ...
      {askRequest && (
        <QuestionPanel req={askRequest} onComplete={(answers) => askRequest.resolve(answers)} />
      )}
      {picker && (
        <ModelPicker
