...
import type { ApprovalRequest, ToolContext, AskRequest, AskAnswer } from "./tools.js";
...
function consoleApprover(mode: Mode, rl: readline.Interface) {
...
function consoleAsker(rl: readline.Interface) {
...
function buildClientOptionsFor(auth: CcrAuth | null, onQuota?: (q: QuotaState) => void) {
  if (auth) {
...
async function runOneShot(args: ParsedArgs, root: string, auth: CcrAuth | null): Promise<number> {
  let client;
  const quotaRef: { current: QuotaState | null } = { current: null };
  try {
    client = buildClient(
      buildClientOptionsFor(auth, (q) => {
        quotaRef.current = q;
      }),
    );
  } catch (e: any) {
...
async function runInteractive(
  ...
  const { waitUntilExit } = render(
    React.createElement(App, {
      ...
      initialPrompt,
      buildClient: (onQuota?: (q: QuotaState) => void) =>
        buildClient(buildClientOptionsFor(auth, onQuota)),
...
async function runLogin(args: ParsedArgs): Promise<number> {
  ...
  try {
    ...
    const browserModule: any = await import(browserModulePath);
    if (typeof browserModule.runBrowserAuth === "function") {
      return browserModule.runBrowserAuth({ method: args.authMethod });
    }
  } catch {}
  console.error(
    kleur.red("Browser login is not available in this build. Run `ccr login --terminal`."),
  );
  return 1;
}
