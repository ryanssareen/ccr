...
interface BrowserAuthOptions {
  endpoint?: string;
  method?: "email" | "github";
  // Test seams. Production callers should never set these.
...
const successPage = (email: string | null): string => `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
async function authenticateViaBrowser(
  ...
  if (!opened) {
    ...
    server.close();
    throw new BrowserAuthError(
      "Could not open a web browser. Run `ccr login --terminal` instead.",
    );
  }
