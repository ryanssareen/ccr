...
import { ReadStream, existsSync, statSync, promises, readFileSync } from "node:fs";
...
let convert;
try {
  convert = require2("encoding").convert;
} catch (e2) {
}
...
class Blob2 {
  constructor(blobParts = [], options = {}) {
    ...
    __classPrivateFieldSet$8(this, _Blob_type, /^[\x20-\x7E]*$/.test(type2) ? type2 : "", "f");
  }
...
let AbortSignal$1 = class AbortSignal2 extends EventTarget {
  /**
...
class FormDataEncoder {
  constructor(form, boundaryOrOptions, options) {
    ...
    __classPrivateFieldSet$6(this, _FormDataEncoder_options, { ...defaultOptions$1, ...options }, "f");
...
async function fileFromPath(path2, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./chunks/fileFromPath-Dv5zSr1y.js");
...
class EventStream {
  constructor() {
    ...
    __classPrivateFieldSet$3(this, _EventStream_connectedPromise, new Promise((resolve2, reject) => {
      __classPrivateFieldSet$3(this, _EventStream_resolveConnectedPromise, resolve2, "f");
      __classPrivateFieldSet$3(this, _EventStream_rejectConnectedPromise, reject, "f");
    }), "f");
    __classPrivateFieldSet$3(this, _EventStream_endPromise, new Promise((resolve2, reject) => {
      __classPrivateFieldSet$3(this, _EventStream_resolveEndPromise, resolve2, "f");
...
class AbstractChatCompletionRunner extends EventStream {
  ...
  async _runTools(client, params, options) {
    ...
    const tools2 = "tools" in params ? inputTools.map((t2) => t2.type === "function" ? {
      type: "function",
      function: {
...
Uploads.Parts = Parts;
const allSettledWithThrow = async (promises2) => {
...
function truncate(s2, limit2 = MAX_TOOL_OUTPUT) {
  if (s2.length <= limit2) return s2;
  return s2.slice(0, limit2) + `

[... truncated ${s2.length - limit2} bytes ...]`;
}
function safePath(root, target) {
  return path.isAbsolute(target) ? path.resolve(target) : path.resolve(root, target);
}
async function readFileText(p2) {
  return promises.readFile(p2, "utf8");
}
...
async function* walk(dir) {
  ...
  for (const e2 of entries) {
    if (e2.isDirectory()) {
      if (SKIP_DIRS.has(e2.name) || e2.name.startsWith(".")) continue;
      yield* walk(path.join(dir, e2.name));
    } else if (e2.isFile()) {
      yield path.join(dir, e2.name);
    }
  }
}
function globToRegex(pattern) {
  const re2 = pattern.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*\*/g, "::DOUBLESTAR::").replace(/\*/g, "[^/]*").replace(/::DOUBLESTAR::/g, ".*").replace(/\?/g, ".");
...
const readFileTool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      limit: { type: "integer", description: "Max lines to return.", default: 2e3 }
    },
    required: ["path"]
  },
  async run(ctx, { path: target, offset = 0, limit: limit2 = 2e3 }) {
    const p2 = safePath(ctx.root, target);
    if (!existsSync(p2)) return `ERROR: file not found: ${p2}`;
    const st2 = statSync(p2);
    if (st2.isDirectory()) return `ERROR: is a directory: ${p2}`;
    if (st2.size > MAX_FILE_BYTES)
      return `ERROR: file too large (${st2.size} bytes); read in chunks via offset/limit`;
    const text = await readFileText(p2);
    const lines = text.split("\n");
    const sliced = lines.slice(offset, offset + limit2);
    const numbered = sliced.map((ln, i2) => `${String(i2 + 1 + offset).padStart(6)}	${ln}`).join("\n");
    return truncate(`${p2} (${lines.length} lines)
${numbered}`);
  }
};
const writeFileTool = {
  ...
  async run(ctx, { path: target, content }) {
    const p2 = safePath(ctx.root, target);
    const existed = existsSync(p2);
    const old = existed ? await readFileText(p2) : "";
    const diff2 = unifiedDiff(old, content, path.relative(ctx.root, p2) || p2);
    const ok = await ctx.approve({
      kind: "edit",
      title: `${existed ? "Overwrite" : "Create"} ${p2}`,
      detail: diff2 || "(empty file)"
    });
    if (!ok) return "DENIED: user rejected the write.";
    await promises.mkdir(path.dirname(p2), { recursive: true });
    await promises.writeFile(p2, content, "utf8");
    return `OK: wrote ${Buffer.byteLength(content, "utf8")} bytes to ${p2}`;
  }
};
const editFileTool = {
  name: "edit_file",
  description: "Replace old_string with new_string in a file. old_string must be unique unless replace_all=true. Requires approval.",
  readOnly: false,
  parameters: {
    type: "object",
    properties: {
  ...
  async run(ctx, { path: target, old_string, new_string, replace_all = false }) {
    ...
    const diff2 = unifiedDiff(text, newText, path.relative(ctx.root, p2) || p2);
    const ok = await ctx.approve({ kind: "edit", title: `Edit ${p2}`, detail: diff2 });
    if (!ok) return "DENIED: user rejected the edit.";
    await promises.writeFile(p2, newText, "utf8");
    return `OK: edited ${p2}`;
  }
};
const multiEditTool = {
  name: "multi_edit",
  description: "Apply a sequence of search-replace edits to a single file atomically. Each edit operates on the result of the previous one. Requires a single approval for the combined diff.",
  readOnly: false,
  parameters: {
    type: "object",
    properties: {
  ...
  async run(ctx, { path: target, edits }) {
    const p2 = safePath(ctx.root, target);
    if (!existsSync(p2)) return `ERROR: file not found: ${p2}`;
    const original = await readFileText(p2);
    let cur = original;
    for (let i2 = 0; i2 < edits.length; i2++) {
      const { old_string, new_string, replace_all = false } = edits[i2];
      if (!cur.includes(old_string)) return `ERROR: edit #${i2 + 1}: old_string not found`;
      const count = cur.split(old_string).length - 1;
      if (!replace_all && count > 1)
        return `ERROR: edit #${i2 + 1}: old_string appears ${count} times; set replace_all=true or expand context`;
    ...
    const diff2 = unifiedDiff(original, cur, path.relative(ctx.root, p2) || p2);
    const ok = await ctx.approve({
      kind: "edit",
      title: `Multi-edit ${p2} (${edits.length} edits)`,
      detail: diff2
    });
    if (!ok) return "DENIED: user rejected the edits.";
    await promises.writeFile(p2, cur, "utf8");
    return `OK: applied ${edits.length} edits to ${p2}`;
  }
};
const insertLinesTool = {
  name: "insert_lines",
  description: "Insert content at a specific 1-indexed line number in a file (content is inserted before that line; use line=N+1 to append). Requires approval.",
  readOnly: false,
  parameters: {
    type: "object",
    properties: {
  ...
  async run(ctx, { path: target, line, content }) {
    ...
    const diff2 = unifiedDiff(text, out, path.relative(ctx.root, p2) || p2);
    const ok = await ctx.approve({
      kind: "edit",
      title: `Insert at line ${line} in ${p2}`,
      detail: diff2
    });
    if (!ok) return "DENIED: user rejected the insert.";
    await promises.writeFile(p2, out, "utf8");
    return `OK: inserted ${insert.length} lines at ${p2}:${line}`;
  }
};
const globTool = {
  name: "glob",
  ...
  async run(ctx, { pattern, path: base }) {
    ...
    const matches = [];
    const re2 = globToRegex(pattern);
    for await (const file of walk(root)) {
      const rel = path.relative(root, file);
      const name = path.basename(file);
      if (re2.test(rel) || re2.test(name)) {
        try {
    ...
    matches.sort((a2, b2) => b2.mtime - a2.mtime);
    return truncate(matches.map((m2) => m2.p).join("\n") || "(no matches)");
  }
};
const grepTool = {
  name: "grep",
  ...
  async run(ctx, { pattern, path: base, glob }) {
    ...
    try {
      ...
      return `ERROR: bad regex: ${e2.message}`;
    }
    const out = [];
    for await (const file of walk(root)) {
...
const bashTool = {
  name: "bash",
  description: "Run a shell command in the project directory. Requires approval. Returns exit code, stdout, stderr.",
  readOnly: false,
  parameters: {
    type: "object",
    properties: {
...
const askUserQuestionTool = {
  ...
  description: "Ask the user 1-3 short clarifying questions when the request is genuinely ambiguous. Each question presents multiple-choice options plus a free-text 'Other' path. Do NOT use for trivial preferences or things you can decide yourself.",
  readOnly: true,
  parameters: {
    type: "object",
    properties: {
      questions: {
        ...
        items: {
          ...
          properties: {
            ...
            options: {
              type: "array",
              description: "2-5 concise multiple-choice options. A free-text 'Other' choice is added automatically.",
              items: { type: "string" }
            }
          },
  ...
  async run(ctx, { questions }) {
    ...
    for (let i2 = 0; i2 < questions.length; i2++) {
      ...
      if (!q2 || typeof q2.question !== "string")
        return `ERROR: question ${i2} missing 'question' field`;
      if (!Array.isArray(q2.options)) return `ERROR: question ${i2} 'options' must be an array`;
    }
    ...
    return questions.map((q2, i2) => `Q: ${q2.question}
A: ${answers?.[i2]?.answer ?? "(no answer)"}`).join("\n\n");
  }
};
const spawnAgentTool = {
  name: "spawn_agent",
  description: "Deploy a focused subagent to handle a self-contained task (multi-step research, parallel investigation, multi-file refactor). The subagent shares your tools and the user's approval flow but runs its own conversation loop. Returns the subagent's final summary as a string. Cannot be called from inside a subagent. Tell the user you are deploying an agent before calling this.",
  readOnly: false,
  parameters: {
    type: "object",
    properties: {
      task: {
        type: "string",
        description: "Self-contained task description for the subagent. Include all the context it needs — it cannot see the parent conversation."
      },
      role: {
        type: "string",
      ...
      model: {
        type: "string",
        description: "Optional model id for the subagent (e.g. 'llama-3.3-70b-versatile'). If omitted, parent's model is used. Ask the user via ask_user_question if you're unsure which model to use."
      },
      reason: {
        type: "string",
...
const TOOL_BY_NAME = Object.fromEntries(
  TOOLS.map((t2) => [t2.name, t2])
);
function toolSchemas(exclude) {
  const filtered = exclude && exclude.size ? TOOLS.filter((t2) => !exclude.has(t2.name)) : TOOLS;
...
async function dispatch(ctx, name, args) {
  ...
  try {
    ...
    return `ERROR: ${e2?.name ?? "Error"}: ${e2?.message ?? String(e2)}`;
  }
}
...
class LockOwnedElsewhereError extends Error {
  pid;
  host;
  constructor(message, pid, host) {
    super(message);
...
function lockPath(sessionPath2) {
  return sessionPath2 + ".lock";
}
...
async function readLock(sessionPath2) {
  const p2 = lockPath(sessionPath2);
  if (!existsSync(p2)) return null;
  try {
    const text = await promises.readFile(p2, "utf8");
    const parsed = JSON.parse(text);
    if (typeof parsed.pid === "number" && typeof parsed.host === "string" && typeof parsed.sessionId === "string" && typeof parsed.startedAt === "string") {
      return parsed;
    }
    return null;
  } catch {
...
async function writeLockAtomic(p2, lock) {
  const tmp = `${p2}.${process.pid}.tmp`;
  await promises.writeFile(tmp, JSON.stringify(lock, null, 2), "utf8");
  await promises.rename(tmp, p2);
  const verify = JSON.parse(await promises.readFile(p2, "utf8"));
...
async function acquireLock(sessionPath2, sessionId, opts = {}) {
  const myPid = opts.pid ?? process.pid;
  const myHost = opts.host ?? hostname();
  const isAlive = opts.isPidAlive ?? defaultIsPidAlive;
  const p2 = lockPath(sessionPath2);
  const existing = await readLock(sessionPath2);
  if (existing) {
    if (existing.host !== myHost) {
      throw new LockOwnedElsewhereError(
        `Session is locked on another machine (host=${existing.host} pid=${existing.pid}). CCR's single-machine assumption refuses cross-host takeover; resolve manually.`,
        existing.pid,
        existing.host
      );
    }
    if (existing.pid === myPid) {
      ...
      await writeLockAtomic(p2, refreshed);
      return refreshed;
    }
    if (isAlive(existing.pid)) {
      throw new LockOwnedElsewhereError(
        `Session is active in another ccr process (pid=${existing.pid}). Close the other window or wait for it to finish.`,
        existing.pid,
        existing.host
      );
    }
  ...
  await writeLockAtomic(p2, fresh);
  return fresh;
}
async function releaseLock(sessionPath2, opts = {}) {
  const myPid = opts.pid ?? process.pid;
  const myHost = opts.host ?? hostname();
  const p2 = lockPath(sessionPath2);
  const existing = await readLock(sessionPath2);
  ...
  try {
    await promises.unlink(p2);
  } catch (e2) {
    if (e2.code !== "ENOENT") throw e2;
  }
}
const GROQ_BASE_URL = "https://api.groq.com/openai/v1";
const PROXY_API_PATH = "/api/v1";
const DEFAULT_MODEL = process.env.CCR_MODEL || "llama-3.3-70b-versatile";
...
function makeQuotaCapturingFetch(onQuota) {
  ...
  return async (input, init2) => {
    ...
    const limit2 = res.headers.get("X-CCR-Quota-Limit");
    const resetAt = res.headers.get("X-CCR-Quota-Reset");
    if (used !== null && limit2 !== null && resetAt !== null) {
      const u2 = Number(used);
      const l2 = Number(limit2);
      const r2 = new Date(resetAt);
      if (Number.isFinite(u2) && Number.isFinite(l2) && !Number.isNaN(r2.getTime())) {
        try {
          onQuota({ used: u2, limit: l2, resetAt: r2 });
        } catch {
        }
      }
    }
...
function initialMessages(root, projectContext) {
...
function shortArgs(args) {
  ...
  for (const [k2, v2] of Object.entries(args ?? {})) {
    ...
    if (s2 && s2.length > 80) s2 = s2.slice(0, 77) + '..."';
    parts.push(`${k2}=${s2}`);
  }
  return parts.join(", ");
}
function compactOldToolResults(messages, keepRecent = 2) {
  ...
  for (let i2 = 0; i2 < messages.length; i2++) {
    if (messages[i2]?.role === "tool") toolIndices.push(i2);
  }
  if (toolIndices.length <= keepRecent) return;
  const cutoff = toolIndices.length - keepRecent;
  for (let k2 = 0; k2 < cutoff; k2++) {
    const i2 = toolIndices[k2];
    const m2 = messages[i2];
    if (typeof m2.content === "string" && m2.content.length > 400 && !m2.content.startsWith("[compacted]")) {
      m2.content = `[compacted] earlier tool result (${m2.content.length} chars). Re-call the tool if you need it again.`;
    }
  }
}
function isRateLimitError(err) {
  ...
  return status === 413 || status === 429 || /Request too large/i.test(msg) || /rate.?limit/i.test(msg) || /tokens per minute/i.test(msg);
}
function parseRetryAfterSeconds(err) {
  const hdrs = err?.headers ?? err?.response?.headers;
  if (hdrs) {
    const get = typeof hdrs.get === "function" ? (k2) => hdrs.get(k2) : (k2) => hdrs[k2];
    const ra = get("retry-after") ?? get("Retry-After");
    if (ra) {
      const n2 = parseFloat(String(ra));
      if (!Number.isNaN(n2) && n2 > 0) return Math.ceil(n2);
    }
    const reset = get("x-ratelimit-reset-tokens") ?? get("X-RateLimit-Reset-Tokens");
    if (reset) {
      const m22 = String(reset).match(/(\d+(?:\.\d+)?)s/);
      if (m22) return Math.ceil(parseFloat(m22[1]));
      const n2 = parseFloat(String(reset));
      if (!Number.isNaN(n2) && n2 > 0) return Math.ceil(n2);
    }
  }
  const msg = err?.message || "";
  const m2 = msg.match(/try again in (\d+(?:\.\d+)?)\s*s/i);
  if (m2) return Math.ceil(parseFloat(m2[1]));
  return 60;
}
function isUnrecoverableSize(err) {
  const msg = err?.message || "";
  const m2 = msg.match(/Limit\s+(\d+)[^\d]+Requested\s+(\d+)/i);
  if (m2) {
    const limit2 = parseInt(m2[1], 10);
    const requested = parseInt(m2[2], 10);
    if (limit2 > 0 && requested > limit2 * 2) return true;
  }
  return false;
}
async function sleepWithCountdown(seconds, reporter, signal) {
  for (let s2 = seconds; s2 > 0; s2--) {
    reporter.setStatus?.(`⏱ rate-limited; retrying in ${s2}s (Ctrl-C to cancel)`);
...
function isToolChoiceNoneMismatch(err) {
  const msg = err?.message || "";
  return /tool[_ ]choice/i.test(msg) && /none/i.test(msg) && /called a tool/i.test(msg);
}
function toolValidationError(err) {
  const msg = err?.message || "";
  const m2 = msg.match(/attempted to call tool '([^']+)' which was not in request\.tools/i);
  if (m2) return m2[1];
  return null;
}
const GREETING_RE = /^(?:hi+|hey+|hello+|yo+|sup|howdy|thanks?(?:\s+(?:you|so\s+much|a\s+lot|man|dude))?|thx|ty|cheers|cool|nice|ok(?:ay)?|got\s*it|sure|sounds\s+good|np|no\s+problem|gm|gn|good\s+(?:morning|night|afternoon|evening)|great|awesome|perfect|amazing|wonderful|excellent|fantastic|alright|right|fine|wow|lol|haha|👋|👍|🙏|❤️)\b[\s.!?,👋👍🙏❤️🤝]*$/i;
const TASK_SIGNALS = /* @__PURE__ */ new Set([
  "read",
  "write",
  "edit",
  "fix",
...
function isLikelyNonTask(text) {
  ...
  if (trimmed.length <= 30) {
    ...
    if (tokens.some((t2) => TASK_SIGNALS.has(t2))) return false;
    return true;
  }
  return false;
}
function looksLikeNonTask(messages) {
  let lastUserIdx = -1;
  ...
  const text = typeof messages[lastUserIdx].content === "string" ? messages[lastUserIdx].content : "";
  if (!isLikelyNonTask(text)) return false;
  for (let j2 = lastUserIdx - 1; j2 >= 0; j2--) {
    const m2 = messages[j2];
    if (!m2) continue;
    if (m2.role === "tool") return false;
    if (m2.role === "assistant" && Array.isArray(m2.tool_calls) && m2.tool_calls.length > 0) {
      return false;
    }
...
async function step(run, messages, forceAuto = false) {
  compactOldToolResults(messages);
  const tools2 = toolSchemas(run.ctx.isSubagent ? /* @__PURE__ */ new Set(["spawn_agent"]) : void 0);
  const tool_choice = !forceAuto && looksLikeNonTask(messages) ? "none" : "auto";
  ...
  let content = "";
  const tcByIndex = /* @__PURE__ */ new Map();
  for await (const chunk of stream) {
  ...
  const toolCalls = [...tcByIndex.entries()].sort((a2, b2) => a2[0] - b2[0]).map(([, v2]) => v2);
...
function pickFallbackModel(current) {
  return TOOL_CALL_FALLBACKS.find((m2) => m2 !== current) ?? TOOL_CALL_FALLBACKS[0];
}
const FIRST_TURN_GREETING_RE$1 = /^(?:hi+|hey+y*|hello+|hallo+o*|hola+|ola+|yo+|sup|howdy|hiya|heya|aloha|greetings|gm|gn)[\s.!?,👋]*$/i;
const FIRST_TURN_REPLY = "Hi — what would you like to work on?";
function isFirstTurnGreeting(messages) {
  for (const m2 of messages) {
    if (m2?.role === "assistant" || m2?.role === "tool") return false;
  }
  let userMsg = null;
  for (let i2 = messages.length - 1; i2 >= 0; i2--) {
    if (messages[i2]?.role === "user") {
  ...
  return FIRST_TURN_GREETING_RE$1.test(text);
}
async function runAgent(run, messages) {
  const lockHeld = run.sessionPath != null;
  if (lockHeld) {
    try {
      await acquireLock(run.sessionPath, run.sessionId ?? "unknown");
    } catch (e2) {
      if (e2 instanceof LockOwnedElsewhereError) {
        run.reporter.assistantTurnEnd(
          `Session is active in another window (pid=${e2.pid}). Close it or wait, then retry.`
        );
        return;
      }
      throw e2;
...
async function runAgentInner(run, messages) {
  ...
  for (let i2 = 0; i2 < MAX_ITERATIONS; i2++) {
    ...
    while (true) {
      try {
        ...
        if (isToolChoiceNoneMismatch(e2)) {
        ...
        if (isRateLimitError(e2)) {
          dropOrphanAssistantToolCalls(messages);
          if (isUnrecoverableSize(e2) || rateLimitRetries >= MAX_RATE_LIMIT_RETRIES) {
            run.reporter.setStatus?.(null);
            run.reporter.assistantTurnEnd(
              isUnrecoverableSize(e2) ? "Request is too large to fit in a single per-minute window even after waiting. Try /clear, switch to a higher-limit model with /model, or trim CLAUDE.md." : `Still rate-limited after ${MAX_RATE_LIMIT_RETRIES} retries. Try /clear or /model to switch to a model with a higher TPM cap.`
            );
            return;
          }
          rateLimitRetries++;
          const wait = Math.min(parseRetryAfterSeconds(e2) + 2, MAX_RETRY_WAIT_SECONDS);
          try {
        ...
        const badTool = toolValidationError(e2);
        if (badTool) {
          dropOrphanAssistantToolCalls(messages);
          const valid = (await Promise.resolve().then(() => tools)).TOOLS.map((t2) => t2.name).join(", ");
          messages.push({
            role: "system",
        ...
        if (isTransientToolCallError(e2) && attempt < 2) {
          attempt++;
          dropOrphanAssistantToolCalls(messages);
          await new Promise((r2) => setTimeout(r2, 400 * attempt));
          continue;
        }
        if (isTransientToolCallError(e2)) {
          dropOrphanAssistantToolCalls(messages);
          if (!fallbackTried) {
...
const SUBAGENT_SYSTEM_PROMPT = (role, root) => `You are a "${role}" subagent inside ccr, deployed by a parent agent to handle one focused task. Reply in 1-3 short paragraphs at the end.
...
Project root: ${root}`;
function makeSubagentRunner(client, parentCtx, defaultModel, parentReporter) {
  return async (opts) => {
...
const SESSIONS_DIR$1 = path.join(os.homedir(), ".ccr", "sessions");
function sessionsRootDirectory() {
  return SESSIONS_DIR$1;
}
function projectId(root) {
  return createHash("sha1").update(path.resolve(root)).digest("hex").slice(0, 12);
}
function projectDir(root) {
  return path.join(SESSIONS_DIR$1, projectId(root));
}
...
async function listSessions(root) {
  ...
  const stats = await Promise.all(
    entries.filter((e2) => e2.endsWith(".json")).map(async (e2) => ({ p: path.join(dir, e2), mtime: (await promises.stat(path.join(dir, e2))).mtimeMs }))
  );
  stats.sort((a2, b2) => b2.mtime - a2.mtime);
  return stats.map((s2) => s2.p);
}
async function loadSession(root, sessionId) {
  ...
  const data = JSON.parse(await promises.readFile(target, "utf8"));
  const pr = typeof data.projectRoot === "string" ? data.projectRoot : void 0;
  return { id: sessionId, messages: data.messages ?? [], projectRoot: pr };
}
async function saveSession(root, sessionId, messages) {
  const p2 = sessionPath(root, sessionId);
  await promises.mkdir(path.dirname(p2), { recursive: true });
...
async function listSessionsIndex() {
  if (!existsSync(SESSIONS_DIR$1)) return [];
  const projectDirs = (await promises.readdir(SESSIONS_DIR$1, { withFileTypes: true })).filter((d2) => d2.isDirectory() && PROJECT_HASH_RE.test(d2.name)).map((d2) => d2.name);
  const out = [];
  for (const hash of projectDirs) {
    const dir = path.join(SESSIONS_DIR$1, hash);
    let names;
    try {
    ...
    for (const name of names) {
      ...
      let st2;
      try {
        st2 = await promises.stat(sessionPathAbs);
      } catch {
        continue;
      }
      ...
      let data;
      try {
        data = JSON.parse(await promises.readFile(sessionPathAbs, "utf8"));
      } catch {
        data = {};
      }
      ...
      out.push({
        ...
        updatedAt: typeof data.updated === "number" && Number.isFinite(data.updated) ? data.updated : st2.mtimeMs,
        messageCount: Array.isArray(data.messages) ? data.messages.length : 0
      });
    }
  }
  out.sort((a2, b2) => b2.updatedAt - a2.updatedAt);
  return out;
}
async function loadSessionByPath(absPath) {
  const sessionId = path.basename(absPath, ".json");
  const data = JSON.parse(await promises.readFile(absPath, "utf8"));
  const pr = typeof data.projectRoot === "string" ? data.projectRoot : void 0;
  return { id: sessionId, messages: data.messages ?? [], projectRoot: pr };
}
const EntryTypes = {
...
function classify(p2) {
  if (p2.endsWith(LOCK_SUFFIX)) {
    return { type: "lock-acquired", sessionPath: p2.slice(0, -".lock".length) };
  }
  if (p2.endsWith(".json")) {
    return { type: "session-changed", path: p2 };
  }
  return null;
}
function watchSessions(rootDir, onEvent) {
  ...
  watcher.on("add", (p2) => {
    const evt = classify(p2);
    if (evt) onEvent(evt);
  });
  watcher.on("change", (p2) => {
    if (p2.endsWith(".json")) onEvent({ type: "session-changed", path: p2 });
  });
  watcher.on("unlink", (p2) => {
    if (p2.endsWith(LOCK_SUFFIX)) {
      onEvent({ type: "lock-released", sessionPath: p2.slice(0, -".lock".length) });
    } else if (p2.endsWith(".json")) {
      onEvent({ type: "session-removed", path: p2 });
    }
  });
  let closed = false;
  return {
...
async function loadConfig() {
  ...
  try {
    const text = await promises.readFile(CONFIG_PATH, "utf8");
    return JSON.parse(text);
  } catch {
    return {};
  }
...
async function saveConfig(cfg) {
  ...
  await promises.writeFile(CONFIG_PATH, JSON.stringify(cfg, null, 2), { mode: 384 });
}
async function loadAuth() {
  if (!existsSync(AUTH_PATH)) return null;
  try {
    const text = await promises.readFile(AUTH_PATH, "utf8");
    const parsed = JSON.parse(text);
    if (typeof parsed.token === "string" && typeof parsed.endpoint === "string" && parsed.token.length > 0 && parsed.endpoint.length > 0) {
      return {
        token: parsed.token,
...
async function clearAuth() {
  if (existsSync(AUTH_PATH)) {
    await promises.unlink(AUTH_PATH);
  }
}
...
const VERSION = "1.4.5";
const PACKAGE_NAME = "@ryanisavibecoder/ccr";
...
function compareSemver(a2, b2) {
  const pa = a2.split(".").map((n2) => parseInt(n2, 10) || 0);
  const pb = b2.split(".").map((n2) => parseInt(n2, 10) || 0);
...
async function loadCache() {
  ...
  try {
    const text = await promises.readFile(CACHE_PATH, "utf8");
    const parsed = JSON.parse(text);
...
async function saveCache(latest) {
  try {
...
const PROJECT_CONTEXT_PER_FILE = 6e3;
const PROJECT_CONTEXT_TOTAL = 12e3;
const FIRST_TURN_GREETING_RE = /^(?:hi+|hey+y*|hello+|hallo+o*|hola+|ola+|yo+|sup|howdy|hiya|heya|aloha|greetings|gm|gn)[\s.!?,👋]*$/i;
const DEFAULT_DEPS = {
...
class AgentHost {
  ...
  async start(sender, input) {
    ...
    await promises.mkdir(path.dirname(sessionFilePath), { recursive: true });
    try {
  ...
  async runSession(sender, input, sessionFilePath, signal) {
  ...
  async loadMessagesForSession(sessionFilePath) {
    if (existsSync(sessionFilePath)) {
      try {
        const parsed = JSON.parse(readFileSync(sessionFilePath, "utf8"));
  ...
  createReporter(sender, sessionId) {
    return {
      ...
      toolCallStart: (name, argsPreview) => sender.send("agent:tool-start", { sessionId, name, argsPreview }),
      toolCallEnd: (name, result, isError) => sender.send("agent:tool-end", { sessionId, name, result, isError }),
  ...
  createToolContext(sender, sessionId, mode, signal) {
  ...
  nextRequestId(kind2) {
    ...
    return `${kind2}-${this.requestCounter}`;
  }
  rejectPendingForSession(sessionId, error) {
...
function buildClientOptionsFor(auth, onQuota) {
  if (auth) {
    return {
...
function isGreetingOnlyFirstTurn(messages, text) {
...
function isLockOwnedElsewhere(error) {
  return Boolean(
    error && typeof error === "object" && "pid" in error && "host" in error && error.name === "LockOwnedElsewhereError"
  );
}
async function loadProjectContext(root) {
  ...
  for (const name of CONTEXT_FILES) {
    ...
    let body;
    try {
      body = await promises.readFile(candidate, "utf8");
    } catch {
      continue;
    }
    let note = "";
...
function createRendererSender(webContents) {
  return {
...
async function readForeignLockPid(sessionPathAbs) {
...
async function buildSessionLoad(sessionPathAbs) {
...
function registerIpcHandlers(ipcMain2, host, options) {
  ipcMain2.handle(CHANNELS.bootstrap, async () => {
    const [{ loadAuth: loadAuth2 }, { loadConfig: loadConfig2 }] = await Promise.all([
      import("./chunks/index-_RhZMs9m.js"),
      import("./chunks/index-_RhZMs9m.js")
    ]);
    const [auth, config] = await Promise.all([loadAuth2(), loadConfig2()]);
    return {
      auth,
      config: config ?? {},
  ...
  ipcMain2.handle(
    CHANNELS.agentStart,
    (event, payload) => host.start(createRendererSender(event.sender), payload)
  );
  ipcMain2.handle(
    CHANNELS.agentAbort,
    (_event, payload) => host.abort(payload)
  );
  ipcMain2.handle(
    CHANNELS.agentApprovalResponse,
    (_event, payload) => host.respondToApproval(payload)
  );
  ipcMain2.handle(
    CHANNELS.agentAskResponse,
    (_event, payload) => host.respondToAsk(payload)
  );
  ipcMain2.handle(CHANNELS.sessionsList, () => buildSessionsList());
  ipcMain2.handle(CHANNELS.sessionsLoad, (_event, payload) => {
    if (typeof payload !== "string") throw new Error("sessions:load expects a string path");
    return buildSessionLoad(payload);
  });
  ipcMain2.handle(CHANNELS.sessionsCreate, (_event, payload) => {
    const input = payload ?? { projectRoot: options.defaultProjectRoot() };
    const root = input.projectRoot || options.defaultProjectRoot();
    const id = newSessionId();
    return {
      sessionId: id,
      sessionPath: sessionPath(root, id)
    };
  });
  ipcMain2.handle(CHANNELS.sessionsTakeoverLock, async (_event, payload) => {
    ...
    try {
      ...
      if (err?.name === "LockOwnedElsewhereError") {
        const lockErr = err;
        return {
          ok: false,
  ...
  ipcMain2.handle(CHANNELS.settingsSave, async (_event, payload) => {
...
function registerSessionWatcher(getWindows) {
  const watcher = watchSessions(SESSIONS_DIR, (evt) => {
...
let disposeIpcHandlers = null;
let disposeSessionWatcher = null;
...
async function createWindow() {
  ...
  if (!app.isPackaged && process.env.ELECTRON_RENDERER_URL) {
    await window2.loadURL(process.env.ELECTRON_RENDERER_URL);
  } else {
    await window2.loadFile(rendererIndexPath());
  }
  return window2;
}
app.whenReady().then(async () => {
  ...
  disposeSessionWatcher = registerSessionWatcher(
    () => BrowserWindow.getAllWindows().filter((w2) => !w2.isDestroyed()).map((w2) => w2.webContents)
  );
  await createWindow();
...
export {
  ...
  configPath as i,
  dispatch as j,
  initialMessages as k,
  listSessions as l,
  listSessionsIndex as m,
  loadAuth as n,
  loadConfig as o,
  loadSession as p,
  loadSessionByPath as q,
  lockPath as r,
  makeSubagentRunner as s,
  newSessionId as t,
  projectId as u,
  readLock as v,
  releaseLock as w,
